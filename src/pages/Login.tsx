...
const Login = () => {
  ...
  const handleSubmit = async (values: LoginFormValues) => {
    ...
    try {
      ...
      // In a real app, you'd have a separate phone login endpoint
      ...
      // Success - redirect based on userRole from localStorage
      const userRole = localStorage.getItem("userRole");
      let defaultRedirect = "/dashboard";
      
      if (userRole === "provider") {
        const providerOnboarded = localStorage.getItem("providerOnboarded");
        defaultRedirect = providerOnboarded === "true" ? "/provider/dashboard" : "/provider/onboarding";
  ...
  const handleGuestContinue = () => {
    // Navigate to search or home - allow guest browsing
    const redirectTo = location.state?.from?.pathname || location.search?.split("redirect=")[1] || "/search";
    navigate(redirectTo, { replace: true });
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-muted/20 p-4 relative">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-foreground mb-2 break-words">{t("welcomeBack")}</h1>
          <p className="text-muted-foreground break-words">{t("loginSubtitle")}</p>
        </div>
            ...
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5" dir={dir}>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    <span className="break-words">{t("login")}</span>
              ...
              <Button
                ...
                variant="outline"
                size="lg"
                className="w-full h-12 text-base font-medium break-words"
